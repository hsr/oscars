...
public class Invoker {
	...
	static private ContextConfig cc = ContextConfig
			.getInstance(ServiceNames.SVC_TOPO);
	...
	public static void main(String[] args) throws Exception {
		...
		OptionParser parser = new OptionParser() {
			{
				...
				accepts("mode", "server / client mode").withRequiredArg()
						.describedAs("client / server (default)")
						.ofType(String.class);
				accepts("c", "context")
						.withRequiredArg()
						.describedAs(
								"TESTING,DEVELOPMENT,SDK,PRODUCTION(default)")
						.ofType(String.class);
			}
		};
		...
		if (options.has("c")) {
			...
			if (!context.equals("UNITTEST") && !context.equals("SDK")
		...
		// TopoBridgeCore defines the file path of the topology, which will be
		// read later by getLocalTopogly method.
